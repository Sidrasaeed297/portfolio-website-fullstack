...
def test_register_duplicate_email(client: TestClient):
    # First registration
    register_user(client, "bob", "bob@example.com", "password123")
    # Duplicate registration
    resp = register_user(client, "bob2", "bob@example.com", "password456")
...
def test_login_invalid_password(client: TestClient):
    register_user(client, "dave", "dave@example.com", "davepwd12")
...
def test_refresh_token_success(client: TestClient):
    register_user(client, "gina", "gina@example.com", "ginapwd1")
    login = login_user(client, "gina@example.com", "ginapwd1")
...
def test_admin_route_failure(client: TestClient):
    register_user(client, "ivan", "ivan@example.com", "ivanpwd1")
    login = login_user(client, "ivan@example.com", "ivanpwd1")
    token = login.json()["access_token"]
    resp = client.get("/api/v1/auth/admin-only", headers={"Authorization": f"Bearer {token}"})
...
def test_refresh_token_cannot_access_me(client: TestClient):
    register_user(client, "jane", "jane@example.com", "janepwd1")
    login = login_user(client, "jane@example.com", "janepwd1")
    refresh = login.json()["refresh_token"]
    # Attempt to use refresh token on /me
    resp = client.get("/api/v1/auth/me", headers={"Authorization": f"Bearer {refresh}"})
    assert resp.status_code == 401
    # The error message should indicate refresh tokens cannot be used as access tokens
    assert resp.json()["detail"] == "Refresh token cannot be used as access token"
